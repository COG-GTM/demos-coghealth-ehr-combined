    ...
    <>
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="relative">
            ...
            {showSearchDropdown && searchResults.length > 0 && (
              <div className="absolute top-full left-0 mt-1 w-64 shadow-lg z-50" style={{ background: 'var(--ehr-surface)', border: '1px solid var(--ehr-border-strong)' }}>
                {searchResults.map((patient) => (
                  <div
                    key={patient.id}
                    onClick={() => selectPatient(patient.id)}
                    className="px-2 py-1.5 cursor-pointer text-[11px]" style={{ color: 'var(--ehr-text)', borderBottom: '1px solid var(--ehr-border)' }}
                    onMouseEnter={e => (e.currentTarget.style.background = 'var(--ehr-surface-hover)')}
            ...
            {showSearchDropdown && searchResults.length === 0 && globalSearch.length >= 2 && (
              <div className="absolute top-full left-0 mt-1 w-64 shadow-lg z-50 p-2 text-[11px]" style={{ background: 'var(--ehr-surface)', border: '1px solid var(--ehr-border-strong)', color: 'var(--ehr-text-muted)' }}>
                No patients found
              </div>
            )}
          </div>
      ...
      <div className="ehr-toolbar flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2 text-[10px]" style={{ color: 'var(--ehr-text-muted)' }}>
          ...
          <span style={{ color: 'var(--ehr-border)' }}>|</span>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden border-t" style={{ borderColor: 'var(--ehr-border)', background: 'var(--ehr-surface)' }}>
  ...
  return (
    <BrowserRouter>
      <div className="h-screen flex flex-col ehr-page-bg" style={{ fontFamily: 'Tahoma, sans-serif' }}>
        ...
        <div className="h-5 border-t flex items-center justify-between px-2 text-[10px]" style={{ background: `linear-gradient(to bottom, var(--ehr-statusbar-from), var(--ehr-statusbar-to))`, borderColor: 'var(--ehr-border)', color: 'var(--ehr-text-muted)' }}>
