...
export function LoadingOverlay({ isLoading, text = 'Loading...' }: LoadingOverlayProps) {
  ...
  return (
    <div className="ehr-scrim absolute inset-0 z-40 flex items-center justify-center pointer-events-auto">
      <div className="ehr-window-shadow flex flex-col items-center bg-chrome border-2 border-line-3 px-6 py-4">
        <svg
          width="32"
          height="32"
          viewBox="0 0 32 32"
        ...
        <span className="text-[11px] text-ink" style={{ fontFamily: 'Tahoma, sans-serif' }}>{text}</span>
      </div>
    </div>
  );
}
