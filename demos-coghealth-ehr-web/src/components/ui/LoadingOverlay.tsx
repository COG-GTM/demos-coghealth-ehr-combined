...
export function LoadingOverlay({ isLoading, text = 'Loading...' }: LoadingOverlayProps) {
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center ehr-sidebar-bg border-2 border-[var(--ehr-border)] px-6 py-4 shadow-md" style={{ boxShadow: '2px 2px 4px rgba(0,0,0,0.3)' }}>
        <svg
          width="32"
          height="32"
          viewBox="0 0 32 32"
        ...
        <span className="text-[11px] text-[var(--ehr-text)]" style={{ fontFamily: 'Tahoma, sans-serif' }}>{text}</span>
      </div>
    </div>
  );
}
