  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className={`relative ${widthClasses[width]} max-h-[90vh] flex flex-col`} style={{ fontFamily: 'Tahoma, sans-serif' }}>
        {/* Window frame */}
        <div className="border-2 shadow-lg flex flex-col" style={{ background: 'var(--ehr-modal-frame-bg)', borderColor: 'var(--ehr-border)', boxShadow: '2px 2px 8px rgba(0,0,0,0.3)' }}>
          {/* Title bar */}
          <div 
          ...
          {/* Content */}
          <div className="flex-1 overflow-auto p-3" style={{ background: 'var(--ehr-modal-content-bg)' }}>
            {children}
          </div>
          
          {/* Footer */}
          {footer && (
            <div className="px-3 py-2 flex justify-end space-x-2" style={{ background: 'var(--ehr-modal-content-bg)', borderTop: '1px solid var(--ehr-border)' }}>
              {footer}
            </div>
          )}
        </div>
...
export function AlertDialog({ isOpen, onClose, title, message, type = 'info' }: AlertDialogProps) {
  ...
  return (
    ...
    >
      <div className="p-2" style={{ background: bgVars[type], border: '1px solid var(--ehr-border)', color: textVars[type] }}>
