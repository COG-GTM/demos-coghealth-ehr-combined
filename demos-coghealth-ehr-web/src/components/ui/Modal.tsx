  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className={`relative ${widthClasses[width]} max-h-[90vh] flex flex-col`} style={{ fontFamily: 'Tahoma, sans-serif' }}>
        {/* Window frame */}
        <div className="bg-surface border-2 border-line-3 shadow-lg flex flex-col ehr-window-shadow">
          {/* Title bar */}
          <div className="ehr-header flex items-center justify-between px-2 py-1">
            ...
            <button 
              onClick={onClose}
              className="w-5 h-5 flex items-center justify-center hover:bg-white/20"
            >
          ...
          {/* Content */}
          <div className="flex-1 overflow-auto p-3 bg-chrome">
            {children}
          </div>
          
          {/* Footer */}
          {footer && (
            <div className="px-3 py-2 bg-chrome border-t border-line-3 flex justify-end space-x-2">
              {footer}
            </div>
          )}
        </div>
...
export function ConfirmDialog({ 
  ...
  return (
    ...
    >
      <p className="text-[11px] text-ink">{message}</p>
    </Modal>
  );
}
