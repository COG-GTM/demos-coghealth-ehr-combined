  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className={`relative ${widthClasses[width]} max-h-[90vh] flex flex-col`} style={{ fontFamily: 'Tahoma, sans-serif' }}>
        {/* Window frame */}
        <div className="border-2 shadow-lg flex flex-col" style={{ background: 'var(--ehr-bg-content)', borderColor: 'var(--ehr-border)', boxShadow: '2px 2px 8px rgba(0,0,0,0.3)' }}>
          {/* Title bar */}
          <div 
            className="flex items-center justify-between px-2 py-1"
            style={{ background: 'var(--ehr-bg-header)' }}
          >
          ...
          {/* Content */}
          <div className="flex-1 overflow-auto p-3" style={{ background: 'var(--ehr-bg-modal-content)' }}>
            {children}
          </div>
          
          {/* Footer */}
          {footer && (
            <div className="px-3 py-2 border-t flex justify-end space-x-2" style={{ background: 'var(--ehr-bg-modal-content)', borderColor: 'var(--ehr-border)' }}>
              {footer}
            </div>
          )}
        </div>
