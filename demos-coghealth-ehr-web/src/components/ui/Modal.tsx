  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className={`relative ${widthClasses[width]} max-h-[90vh] flex flex-col`} style={{ fontFamily: 'Tahoma, sans-serif' }}>
        {/* Window frame */}
        <div className="border-2 border-[var(--ehr-border)] shadow-lg flex flex-col ehr-content-bg" style={{ boxShadow: '2px 2px 8px rgba(0,0,0,0.3)' }}>
          {/* Title bar */}
          <div 
          ...
          {/* Content */}
          <div className="flex-1 overflow-auto p-3 ehr-sidebar-bg">
            {children}
          </div>
          
          {/* Footer */}
          {footer && (
            <div className="px-3 py-2 ehr-sidebar-bg border-t border-[var(--ehr-border)] flex justify-end space-x-2">
              {footer}
            </div>
          )}
        </div>
...
export function ConfirmDialog({ 
  ...
  return (
    ...
    >
      <p className="text-[11px] text-[var(--ehr-text)]">{message}</p>
    </Modal>
  );
}
