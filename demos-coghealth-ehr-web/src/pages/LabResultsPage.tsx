...
export default function LabResultsPage() {
  ...
  const getStatusClass = (status: LabResult['status']) => {
    switch (status) {
      case 'critical':
        return 'ehr-alert-critical font-bold';
      case 'abnormal':
          ...
          {filteredPanels.length === 0 ? (
            ...
            filteredPanels.map(panel => (
              <div key={panel.id} className="border-b border-gray-300">
                <div
                  className="flex items-center justify-between px-2 py-1.5 ehr-bar ehr-bar-hover cursor-pointer"
                  ...
                  <table className="w-full text-[11px]">
                    ...
                    <tbody>
                      {panel.results.map((result, idx) => (
                        <tr
                          key={result.id}
                          className={`cursor-pointer ehr-row-hover ${idx % 2 === 0 ? 'bg-white' : 'ehr-row-alt'} ${getStatusClass(result.status)}`}
