...
export default function LabResultsPage() {
  ...
  const getStatusClass = (status: LabResult['status']) => {
    switch (status) {
      case 'critical':
        return 'ehr-status-critical';
      case 'abnormal':
        return 'ehr-status-abnormal';
      default:
        return '';
    }
  };
          ...
          {filteredPanels.length === 0 ? (
            ...
            filteredPanels.map(panel => (
              <div key={panel.id} className="border-b border-gray-300">
                <div
                  className="flex items-center justify-between px-2 py-1.5 bg-gradient-to-b from-[#f8f8f8] to-[#e8e8e8] dark:from-[#333a45] dark:to-[#2a3038] cursor-pointer hover:from-[#fff] hover:to-[#f0f0f0] dark:hover:from-[#3a414c] dark:hover:to-[#333a45]"
                  ...
                  <table className="w-full text-[11px]">
                    ...
                    <tbody>
                      {panel.results.map((result, idx) => (
                        <tr
                          key={result.id}
                          className={`cursor-pointer hover:bg-[#e0e8f0] dark:hover:bg-[#2a3a4d] ${idx % 2 === 0 ? 'bg-white' : 'bg-[#f8f8f8] dark:bg-[#22272f]'} ${getStatusClass(result.status)}`}
