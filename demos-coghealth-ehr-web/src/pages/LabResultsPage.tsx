          ...
          {filteredPanels.length === 0 ? (
            ...
            filteredPanels.map(panel => (
              <div key={panel.id} className="border-b border-gray-300">
                <div
                  className="flex items-center justify-between px-2 py-1.5 ehr-subheader cursor-pointer"
                  ...
                  <table className="w-full text-[11px]">
                    ...
                    <tbody>
                      {panel.results.map((result, idx) => (
                        <tr
                          key={result.id}
                          className={`cursor-pointer ehr-grid-row ${result.status === 'normal' ? (idx % 2 === 0 ? 'bg-white' : 'bg-gray-50') : getStatusClass(result.status)}`}
