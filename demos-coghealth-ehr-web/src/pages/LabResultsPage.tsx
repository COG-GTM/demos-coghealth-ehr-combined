          ...
          {filteredPanels.length === 0 ? (
            ...
            filteredPanels.map(panel => (
              <div key={panel.id} className="border-b border-gray-300">
                <div
                  className="flex items-center justify-between px-2 py-1.5 bg-gradient-to-b from-(--ehr-grad-top) to-(--ehr-surface-2) cursor-pointer hover:from-(--ehr-surface-alt) hover:to-(--ehr-grad-top-2)"
                  ...
                  <table className="w-full text-[11px]">
                    ...
                    <tbody>
                      {panel.results.map((result, idx) => (
                        <tr
                          key={result.id}
                          className={`cursor-pointer hover:bg-(--ehr-hover) ${idx % 2 === 0 ? 'bg-white' : 'bg-(--ehr-surface-alt)'}`}
