...
export default function LabResultsPage() {
  ...
  const getStatusStyle = (status: LabResult['status']) => {
    switch (status) {
      case 'critical':
        return { background: 'var(--ehr-status-critical-bg)', color: 'var(--ehr-status-critical-fg)', fontWeight: 'bold' };
      case 'abnormal':
        return { background: 'var(--ehr-status-abnormal-bg)', color: 'var(--ehr-status-abnormal-fg)' };
      default:
        return {};
    }
  };
          ...
          {filteredPanels.length === 0 ? (
            ...
            filteredPanels.map(panel => (
              <div key={panel.id} className="border-b border-gray-300">
                <div
                  className="flex items-center justify-between px-2 py-1.5 bg-gradient-to-b from-[var(--ehr-bar-from)] to-[var(--ehr-bar-to)] cursor-pointer hover:from-[var(--ehr-bar-hover-from)] hover:to-[var(--ehr-bar-hover-to)]"
                  ...
                  <table className="w-full text-[11px]">
                    ...
                    <tbody>
                      {panel.results.map((result, idx) => (
                        <tr
                          key={result.id}
                          className={`cursor-pointer hover:bg-[var(--ehr-row-hover)] ${idx % 2 === 0 ? 'bg-white' : 'bg-[var(--ehr-row-alt)]'}`}
