...
export default function VitalsPage() {
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      case 'critical': return { background: 'var(--ehr-critical-bg)', color: 'var(--ehr-critical-fg)', fontWeight: 'bold' };
      case 'abnormal': return { background: 'var(--ehr-warning-bg)', color: 'var(--ehr-warning-fg)' };
      default: return {};
    }
  };
        ...
        <div className="flex-1 overflow-auto bg-white border border-gray-400">
          <table className="w-full text-[11px]">
            <thead className="sticky top-0">
              <tr className="bg-gradient-to-b from-(--ehr-grad-top-2) to-(--ehr-grad-bottom-2)">
                <th className="text-left px-2 py-1 border border-gray-400 bg-gradient-to-b from-(--ehr-grad-top) to-(--ehr-surface-2) sticky left-0 z-10 min-w-[100px]">Vital Sign</th>
                <th className="text-center px-2 py-1 border border-gray-400 bg-gradient-to-b from-(--ehr-grad-top) to-(--ehr-surface-2) min-w-[50px]">Trend</th>
                {vitals.map((reading) => (
            ...
            <tbody>
              {vitalSigns.map((vital, vitalIdx) => (
                <tr key={vital.key} className={vitalIdx % 2 === 0 ? 'bg-white' : 'bg-(--ehr-surface-alt)'}>
                  ...
                  {vitals.map((reading, readingIdx) => {
                    ...
                    return (
                      <td
                        key={reading.id}
                        className="px-2 py-1 border border-gray-300 text-center cursor-pointer hover:bg-(--ehr-hover)"
