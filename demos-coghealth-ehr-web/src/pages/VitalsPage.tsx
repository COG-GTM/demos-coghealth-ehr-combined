...
export default function VitalsPage() {
  ...
  const getStatusClass = (status: string) => {
    switch (status) {
      case 'critical': return 'ehr-alert-critical font-bold';
      case 'abnormal': return 'ehr-alert-warning';
      default: return '';
    }
  };
        ...
        <div className="flex-1 overflow-auto bg-white border border-gray-400">
          <table className="w-full text-[11px]">
            <thead className="sticky top-0">
              <tr className="ehr-thead-row">
                <th className="text-left px-2 py-1 border border-gray-400 ehr-bar sticky left-0 z-10 min-w-[100px]">Vital Sign</th>
                <th className="text-center px-2 py-1 border border-gray-400 ehr-bar min-w-[50px]">Trend</th>
                {vitals.map((reading) => (
            ...
            <tbody>
              {vitalSigns.map((vital, vitalIdx) => (
                <tr key={vital.key} className={vitalIdx % 2 === 0 ? 'bg-white' : 'ehr-row-alt'}>
                  ...
                  {vitals.map((reading, readingIdx) => {
                    ...
                    return (
                      <td
                        key={reading.id}
                        className={`px-2 py-1 border border-gray-300 text-center cursor-pointer ehr-row-hover ${getStatusClass(status)}`}
