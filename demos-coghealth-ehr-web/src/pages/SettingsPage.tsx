  ...
  return (
    <div className="h-full flex flex-col" style={{ background: 'var(--ehr-bg)' }}>
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Left Navigation */}
        <div className="w-48 overflow-auto p-2 space-y-1" style={{ background: 'var(--ehr-surface-raised)' }}>
          {tabs.map((tab) => {
            const Icon = tab.icon;
            return (
              <button
        ...
        {/* Right Content */}
        <div className="flex-1 overflow-auto p-3" style={{ background: 'var(--ehr-surface-white)', borderLeft: '1px solid var(--ehr-border)' }}>
          {activeTab === 'profile' && (
            ...
            <div className="space-y-3">
              <fieldset className="ehr-fieldset">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {([['light', Sun], ['dark', Moon], ['system', Monitor]] as const).map(([theme, Icon]) => (
                    <button
                      key={theme}
                      onClick={() => {
                      ...
                      style={
                        themePreference === theme
                          ? { border: '2px solid var(--ehr-btn-primary-from)', background: 'var(--ehr-surface-white)' }
                          : { border: '1px solid var(--ehr-border)', background: 'var(--ehr-surface)' }
                      }
                    >
                      <Icon className="w-4 h-4 mx-auto mb-1" style={{ color: 'var(--ehr-text-muted)' }} />
